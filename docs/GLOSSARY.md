...
| **Citation** | A bracketed number (`[1]`, `[2]`) in a generated answer, pointing back to one specific retrieved chunk's provenance metadata. Rendered from the real `Chunk` the model was shown, never from the model's own text, so the displayed source is always correct even if the model's chosen number is not. *(Ch10)* |
...
| **Generation** | The online step where the LLM produces an answer from a prompt built out of the retrieved context and the user's question. The second half of RAG, after *retrieval*. *(Ch10)* |
