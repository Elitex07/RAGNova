...
- :class:`~src.models.chunk.Chunk`                          — output data model
...
>>> for chunk in chunks:
...     print(chunk.id, len(chunk.content), "chars of OCR text")
"""
...
class ChunkStore:
    """In-memory store for ingested :class:`~src.models.chunk.Chunk` objects.
    ...
    ----------
    deduplicate:
        When *True*, re-inserting a chunk whose ``id`` already exists in the
    ...
    def add(self, chunk: Chunk) -> bool:
        ...
        ----------
        chunk:
            The :class:`~src.models.chunk.Chunk` to store.
        ...
        """
        if self.deduplicate and chunk.id in self._store:
            logger.debug("Skipping duplicate chunk id=%s", chunk.id)
            return False
...
class ImageIngestionPipeline:
    ...
    def ingest_image(
        ...
        """Processes a single image and produces exactly one :class:`~src.models.chunk.Chunk`.

        Processing steps
        ~~~~~~~~~~~~~~~~
        1. Load image and extract file-level metadata.
        2. Run Tesseract OCR → populate ``chunk.content`` with extracted text.
        ...
        -------
        Chunk
            A :class:`~src.models.chunk.Chunk` with:
            - ``modality="image"``
            - ``embedding_model="ViT-B-32/laion2b_s34b_b79k"``
            - ``content`` = OCR-extracted text (empty string if OCR disabled)
            - ``embedding`` = 512-d float vector (``None`` if embeddings disabled)
        ...
        # Step 4 — Build Chunk
        chunk = Chunk(
            id=self._make_chunk_id(img_meta),
            content=ocr_result.text,          # OCR text becomes searchable content
            modality="image",
            embedding=embedding,
        ...
        logger.info(
            "Ingested chunk id=%s  ocr_words=%d  embedding=%s",
            chunk.id,
...
def ingest_image(
    ...
    """Convenience function: ingest a single image into a :class:`~src.models.chunk.Chunk`.
