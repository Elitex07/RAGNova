...
# The report footer below prints box-drawing characters (─). On an
# unconfigured Windows console (cp1252) that raises UnicodeEncodeError
# and kills the script after every check has already run — confirmed by
# actually running this file, not assumed. Same fix, same reason, as
# scripts/verify_setup.py's own stdout reconfigure elsewhere in this repo.
...
with check("7. ImageIngestionPipeline — ingest_image (PIL input)"):
    ...
    print(f"     chunk.chunk_id       : {chunk.chunk_id}")
    print(f"     chunk.source         : {chunk.source}")
    print(f"     chunk.modality       : {chunk.modality}")
    print(f"     chunk.embedding_model: {chunk.embedding_model}")
    print(f"     chunk.text[:60]      : {chunk.text[:60]!r}")
...
with check("9. Chunk.to_dict / to_json / from_dict round-trip"):
    ...
    assert from_dict_chunk.chunk_id == chunk.chunk_id
